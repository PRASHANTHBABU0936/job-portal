...
const jobSlice = createSlice({
  ...
  reducers: {
    ...
    successForAllJobs(state, action) {
      ...
      state.pagination = { // Adding ad-hoc pagination state if possible, or just ignore
        page: action.payload.page,
        totalPages: action.payload.totalPages,
        count: action.payload.count
      };
      // If we strictly can't add new state keys without defining them in initialState, we should update initialState too.
      // But redux-toolkit allows adding keys unless using TS or strict typing. Javascript is flexible.
      state.error = null;
    },
...
export const fetchJobs =
  (city, niche, searchKeyword = "", page = 1) =>
    async (dispatch) => {
      try {
        dispatch(jobSlice.actions.requestForAllJobs());
        let link = `${config.API_URL}/api/v1/job/getall?page=${page}&limit=25`;
        let queryParams = [];
        if (searchKeyword) {
        ...
        // response.data now includes jobs, count, page, totalPages
        // We might want to store totalPages in the state, but jobSlice structure is fixed.
        // For now, let's just update jobs. If we need totalPages in UI, we should add it to slice.
...
export const fetchSingleJob = (jobId) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${config.API_URL}/api/v1/job/get/${jobId}`,
...
export const postJob = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${config.API_URL}/api/v1/job/post`,
      data,
...
export const getMyJobs = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${config.API_URL}/api/v1/job/getmyjobs`,
      { withCredentials: true }
    );
...
export const deleteJob = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `${config.API_URL}/api/v1/job/delete/${id}`,
      { withCredentials: true }
    );
